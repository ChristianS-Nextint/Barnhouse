...
document.addEventListener('shopacado:discounts:update', (data) => {
  // console.log(data.detail.discountResponse.discounts.discounted_price);

  const freeShippingPrice = parseFloat(document.querySelector('.free-shipping').getAttribute('data-total-price'));
  const discountPrice = parseFloat(data.detail.discountResponse.discounts.discounted_price)*100;
  
  if(discountPrice >= freeShippingPrice) {
    document.querySelector('.free-shipping__text-success').classList.remove('hide');
    document.querySelector('.free-shipping__text-remain').classList.add('hide');
    
  }else{
    let price_html  = "<bdi>"+  +"<sup class=\"price__suffix test\">,</sup><span class=\"price__prefix test3\">€</span></bdi>";
    const remain_price = parseFloat((freeShippingPrice - discountPrice)/100).toFixed(2);
    
    const remain_price_html = document.querySelector('.free-shipping__text-remain').querySelector('.price');
    remain_price_html.innerHTML = `<bdi>${theme.Currency.formatMoney(freeShippingPrice - discountPrice, window.shopSettings.moneyFormat)}</bdi>`;

    const progess_percent = Math.round(discountPrice/freeShippingPrice *100);
    document.querySelector('.free-shipping__progress--remain').style.setProperty('--progress', progess_percent + '%');

    document.querySelector('.free-shipping__text-success').classList.add('hide');
    document.querySelector('.free-shipping__text-remain').classList.remove('hide');
