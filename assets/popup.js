...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const form = document.querySelector('.popup__newsletter');

    // Add an event listener to the checkbox for change events
    consentCheckbox.addEventListener('change', function() {
      // Check if the checkbox is now checked
      if (consentCheckbox.checked) {
        // If checked, remove the CSS class
        checkboxLabel.classList.remove('consent-checkbox-label');
      }
    });

    form.addEventListener('submit', function(event) {
      // Check if the checkbox is not checked on form submission
      if (!consentCheckbox.checked) {
        event.preventDefault(); // Prevent form submission
        checkboxLabel.classList.add('consent-checkbox-label'); // Add the CSS class
        const errorMessage = document.querySelector('.form__message.errors');
        errorMessage.style.display = 'block';
        errorMessage.textContent = 'Ich willige in die Erhebung meiner Daten für den Newsletter-Versand ein.';
      }
    });
  });